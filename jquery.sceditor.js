...
;(function($, window, document) {
	...
	var _tmpl = function(name, params, createHTML) {
			...
			if (createHTML) template = $(template);
	...
	$.sceditor = function(el, options) {
		...
		var init, replaceEmoticons, handleCommand, saveRange, handlePasteEvt, handlePasteData, handleKeyPress, handleFormReset, handleMouseDown, initEditor, initToolBar, initKeyPressFuncs, initResize, documentClickHandler, formSubmitHandler, initEmoticons, getWysiwygDoc, handleWindowResize, initLocale, updateToolBar, sourceEditorSelectedText, appendNewLine, autofocus;
		...
		init = function() {
			...
			// Load locale
			if (base.opts.locale && base.opts.locale !== "en") initLocale();

			// if either width or height are % based, add the resize handler to update the editor
			// when the window is resized
			var h = base.opts.height,
				w = base.opts.width;
			if ((h && (h + "").indexOf("%") > -1) || (w && (w + "").indexOf("%") > -1)) $(window).resize(handleWindowResize);
			...
			if (base.opts.resizeEnabled) initResize();

			if (base.opts.id) $editorContainer.attr('id', base.opts.id);

			$(document).click(documentClickHandler);
			$(original.form).attr('novalidate', 'novalidate').bind("reset", handleFormReset).submit(formSubmitHandler);

			// load any textarea value into the editor
			base.val($original.hide().val());

			if (base.opts.autofocus) autofocus();

			// force into source mode if is a browser that can't handle
			// full editing
			if (!$.sceditor.isWysiwygSupported) base.toggleSourceMode();

			if (base.opts.toolbar.indexOf('emoticon') !== -1) initEmoticons();

			// Can't use load event as it gets fired before CSS is loaded
			// in some browsers
			if (base.opts.autoExpand) var interval = setInterval(function() {
		...
		initEditor = function() {
			...
			if (window.location.protocol === "https:") $wysiwygEditor.attr("src", "javascript:false");
			...
			if ($.sceditor.ie) $doc.find("html").addClass('ie').addClass('ie' + $.sceditor.ie);

			// iframe overflow fix
			if (/iPhone|iPod|iPad| wosbrowser\//i.test(navigator.userAgent) || $.sceditor.ie) $body.height('100%');

			// set the key press event
			$body.keypress(handleKeyPress).keyup(appendNewLine);
			$doc.keypress(handleKeyPress).mousedown(handleMouseDown).bind("beforedeactivate keyup", saveRange).keyup(appendNewLine).focus(function() {
				lastRange = null;
			});
			...
			if (base.opts.enablePasteFiltering) $body.bind("paste", handlePasteEvt);

			if (base.opts.autoExpand) $doc.bind("keyup", base.expandToContent);
		...
		initToolBar = function() {
			var $group, $button, buttons, i, x, buttonClick, groups = base.opts.toolbar.split("|");

			buttonClick = function() {
				var $this = $(this);

				if (!$this.hasClass('disabled')) handleCommand($this, base.commands[$this.data("sceditor-command")]);
			...
			for (i = 0; i < groups.length; i++) {
				...
				for (x = 0; x < buttons.length; x++) {
					...
					if (!base.commands[buttons[x]]) continue;

					$button = _tmpl("toolbarButton", {
						name: buttons[x],
						dispName: base.commands[buttons[x]].tooltip || buttons[x]
					}, true).click(buttonClick).data('sceditor-txtmode', !! base.commands[buttons[x]].txtExec).data('sceditor-wysiwygmode', !! base.commands[buttons[x]].exec);

					if (base.commands[buttons[x]].tooltip) $button.attr('title', base._(base.commands[buttons[x]].tooltip));

					if (!base.commands[buttons[x]].exec) $button.addClass('disabled');
			...
			if (base.opts.toolbarContainer) $(base.opts.toolbarContainer).append($toolbar);
			else $editorContainer.append($toolbar);
		};
		 ...
		 */
		autofocus = function() {
			var rng, doc = wysiwygEditor.contentWindow.document,
				body = doc.body;

			if (!doc.createRange) return base.focus();
		...
		base.readOnly = function(readOnly) {
			if (typeof readOnly !== 'boolean') return $sourceEditor.attr('readonly') === 'readonly';

			getWysiwygDoc().body.contentEditable = !readOnly;

			if (!readOnly) $sourceEditor.removeAttr('readonly');
			else $sourceEditor.attr('readonly', 'readonly');
		...
		updateToolBar = function(disable) {
			...
			$toolbar.find('.sceditor-button').removeClass('disabled').each(function() {
				...
				if (disable === true || (inSourceMode && !button.data('sceditor-txtmode'))) button.addClass('disabled');
				else if (!inSourceMode && !button.data('sceditor-wysiwygmode')) button.addClass('disabled');
			});
		};
		...
		initKeyPressFuncs = function() {
			$.each(base.commands, function(command, values) {
				if (values.keyPress) keyPressFuncs.push(values.keyPress);
			});
		};
		...
		base.width = function(width) {
			if (!width) return $editorContainer.width();
		...
		base.height = function(height) {
			if (!height) return $editorContainer.height();
		...
		base.expandToContent = function(ignoreMaxHeight) {
			...
			if (ignoreMaxHeight !== true && height > maxHeight) height = maxHeight;

			if (height > currentHeight) base.height(height);
		};
		 ...
		 */
		initResize = function() {
			var minHeight, maxHeight, minWidth, maxWidth, mouseMoveFunc, $grip = $('<div class="sceditor-grip" />'),
			...
			mouseMoveFunc = function(e) {
				...
				if (base.opts.resizeWidth && newWidth >= minWidth && (maxWidth < 0 || newWidth <= maxWidth)) base.width(newWidth);

				if (base.opts.resizeHeight && newHeight >= minHeight && (maxHeight < 0 || newHeight <= maxHeight)) base.height(newHeight);
		...
		formSubmitHandler = function(e) {
			...
			if (this.checkValidity && !this.checkValidity()) e.preventDefault();
		...
		base.destroy = function() {
			...
			$(original.form).removeAttr('novalidate').unbind('submit', formSubmitHandler).unbind("reset", handleFormReset);
		...
		base.createDropDown = function(menuItem, dropDownName, content, ieUnselectable) {
			...
			$dropdown = $('<div class="sceditor-dropdown sceditor-' + dropDownName + '" />').css(css).append(content).appendTo($('body')).click(function(e) {
		...
		documentClickHandler = function(e) {
			// ignore right clicks
			if (e.which !== 3) base.closeDropDown();
		};

		handlePasteEvt = function(e) {
			var html, elm = getWysiwygDoc().body,
				checkCount = 0,
		...
		handlePasteData = function(elm, pastearea) {
			...
			if (base.opts.getHtmlHandler) pasteddata = base.opts.getHtmlHandler(pasteddata, $(pastearea));

			pastearea.parentNode.removeChild(pastearea);

			if (base.opts.getTextHandler) pasteddata = base.opts.getTextHandler(pasteddata, true);
		...
		getWysiwygDoc = function() {
			if (wysiwygEditor.contentDocument) return wysiwygEditor.contentDocument;

			if (wysiwygEditor.contentWindow && wysiwygEditor.contentWindow.document) return wysiwygEditor.contentWindow.document;

			if (wysiwygEditor.document) return wysiwygEditor.document;
		...
		base.wysiwygEditorInsertHtml = function(html, endHtml, overrideCodeBlocking) {
			...
			if (!overrideCodeBlocking && ($(rangeHelper.parentNode()).is('code') || $(rangeHelper.parentNode()).parents('code').length !== 0)) return;
		...
		base.insertText = function(text, endText) {
			if (base.inSourceMode()) base.sourceEditorInsertText(text, endText);
			else base.wysiwygEditorInsertText(text, endText);
		...
		base.sourceEditorInsertText = function(text, endText) {
			...
			if (typeof sourceEditor.selectionStart !== "undefined") {
				...
				if (endText) text += sourceEditor.value.substring(start, end) + endText;

				sourceEditor.value = sourceEditor.value.substring(0, start) + text + sourceEditor.value.substring(end, sourceEditor.value.length);

				if (endText) sourceEditor.selectionStart = (start + text.length) - endText.length;
				else sourceEditor.selectionStart = start + text.length;

				sourceEditor.selectionEnd = sourceEditor.selectionStart;
			} else if (typeof document.selection.createRange !== "undefined") {
				range = document.selection.createRange();

				if (endText) text += range.text + endText;

				range.text = text;

				if (endText) range.moveEnd('character', 0 - endText.length);

				range.moveStart('character', range.End - range.Start);
				range.select();
			} else sourceEditor.value += text + endText;
		...
		base.val = function(val, filter) {
			if (typeof val === "string") {
				if (base.inSourceMode()) base.setSourceEditorValue(val);
				else {
					if (filter !== false && base.opts.getTextHandler) val = base.opts.getTextHandler(val);
		...
		base.insert = function(start, end, filter, convertEmoticons) {
			if (base.inSourceMode()) base.sourceEditorInsertText(start, end);
			else {
				if (end) {
				...
				if (filter !== false && base.opts.getTextHandler) start = base.opts.getTextHandler(start, true);

				if (convertEmoticons !== false) start = replaceEmoticons(start);
		...
		base.getWysiwygEditorValue = function(filter) {
			...
			if (filter !== false && base.opts.getHtmlHandler) html = base.opts.getHtmlHandler(html, $body);
		...
		base.setWysiwygEditorValue = function(value) {
			if (!value) value = '<p>' + ($.sceditor.ie ? '' : '<br />') + '</p>';
		...
		replaceEmoticons = function(html) {
			if (base.opts.toolbar.indexOf('emoticon') === -1) return html;
			...
			$.each(emoticons, function(key, url) {
				...
				html = html.replace(
				new RegExp(reg, 'gm'), group + _tmpl('emoticon', {
					key: key,
					url: url
				}));
			});
		...
		base.sourceMode = function(enable) {
			if (typeof enable !== 'boolean') return base.inSourceMode();

			if ((base.inSourceMode() && !enable) || (!base.inSourceMode() && enable)) base.toggleSourceMode();
		...
		base.toggleSourceMode = function() {
			...
			if (!$.sceditor.isWysiwygSupported && base.inSourceMode()) return;

			if (base.inSourceMode()) base.setWysiwygEditorValue(base.getSourceEditorValue());
			else base.setSourceEditorValue(base.getWysiwygEditorValue());

			lastRange = null;
			$sourceEditor.toggle();
			$wysiwygEditor.toggle();

			if (!base.inSourceMode()) $editorContainer.removeClass('wysiwygMode').addClass('sourceMode');
			else $editorContainer.removeClass('sourceMode').addClass('wysiwygMode');
		...
		sourceEditorSelectedText = function() {
			...
			if (sourceEditor.selectionStart != null) return sourceEditor.value.substring(sourceEditor.selectionStart, sourceEditor.selectionEnd);
			else if (document.selection.createRange) return document.selection.createRange().text;
		};
		...
		handleCommand = function(caller, command) {
			...
			if (base.inSourceMode()) {
				if (command.txtExec) {
					if ($.isArray(command.txtExec)) base.sourceEditorInsertText.apply(base, command.txtExec);
					else command.txtExec.call(base, caller, sourceEditorSelectedText());
				}
			...
			if ($.isFunction(command.exec)) command.exec.call(base, caller);
			else base.execCommand(command.exec, command.hasOwnProperty("execParam") ? command.execParam : null);
		};
		...
		base.blur = function() {
			...
			if (!$blurElm) $blurElm = $('<input style="width:0; height:0; opacity:0; filter: alpha(opacity=0)" type="text" />').appendTo($editorContainer);
		...
		base.execCommand = function(command, param) {
			...
			if ($parentNode.is('code') || $parentNode.parents('code').length !== 0) return;
			...
			if (!executed && base.commands[command] && base.commands[command].errorMessage) alert(base._(base.commands[command].errorMessage));
		};
		...
		appendNewLine = function() {
			var name, inblock, doc = getWysiwygDoc();

			$.sceditor.dom.rTraverse(doc.body, function(node) {
				name = node.nodeName.toLowerCase();

				if (name === "code" || name === "blockquote") inblock = true;

				// find the last non-empty text node or line break.
				if ((node.nodeType === 3 && !/^\s*$/.test(node.nodeValue)) || node.nodeName.toLowerCase() === 'br' || ($.sceditor.ie && !node.firstChild && !$.sceditor.dom.isInline(node, false))) {
					// this is the last text or br node, if its in a code or quote tag
					// then add a newline to the end of the editor
					if (inblock) $(doc.body).append($('<div>' + (!$.sceditor.ie ? '<br />' : '') + '</div>\n'));
		...
		handleWindowResize = function() {
			if (base.opts.height && base.opts.height.toString().indexOf("%") > -1) base.height($editorContainer.parent().height() * (parseFloat(base.opts.height) / 100));

			if (base.opts.width && base.opts.width.toString().indexOf("%") > -1) base.width($editorContainer.parent().width() * (parseFloat(base.opts.width) / 100));
		};
		...
		base._ = function() {
			...
			if (locale && locale[args[0]]) args[0] = locale[args[0]];

			return args[0].replace(/\{(\d+)\}/g, function(str, p1) {
				return typeof args[p1 - 0 + 1] !== "undefined" ? args[p1 - 0 + 1] : '{' + p1 + '}';
			});
		};
		 ...
		 */
		initLocale = function() {
			if ($.sceditor.locale[base.opts.locale]) locale = $.sceditor.locale[base.opts.locale];
			else {
				var lang = base.opts.locale.split("-");

				if ($.sceditor.locale[lang[0]]) locale = $.sceditor.locale[lang[0]];
			}

			if (locale && locale.dateFormat) base.opts.dateFormat = locale.dateFormat;
		};
	...
	$.sceditor.ie = (function() {
		...
		if ((document.all && window.atob)) v = 10;
	...
	$.sceditor.isWysiwygSupported = (function() {
		...
		if (!contentEditableSupported) return false;
		...
		if (/iPhone|iPod|iPad/i.test(userAgent)) isUnsupported = !/OS [5-9](_\d)+ like Mac OS X/i.test(userAgent);

		// FireFox dose support WYSIWYG on mobiles so override
		// any previous value if using FF
		if (/fennec/i.test(userAgent)) isUnsupported = false;
	...
	$.sceditor.commands = {
		...
		font: {
			...
			exec: function(caller) {
				...
				editor, caller, function(fontName) {
		...
		size: {
			...
			exec: function(caller) {
				...
				editor, caller, function(fontSize) {
		...
		pastetext: {
			exec: function(caller) {
				...
				content.find('.button').click(function(e) {
					...
					if (val) editor.wysiwygEditorInsertText(val);
		...
		image: {
			exec: function(caller) {
				...
				content.find('.button').click(function(e) {
					...
					if (val && val !== "http://") editor.wysiwygEditorInsertHtml('<img' + attrs + ' src="' + val + '" />');
			...
			exec: function(caller) {
				...
				content.find('.button').click(function(e) {
					...
					if (val) {
						...
						if (!editor.getRangeHelper().selectedHtml()) editor.wysiwygEditorInsertHtml('<a href="' + 'mailto:' + val + '">' + val + '</a>');
			...
			exec: function(caller) {
				...
				content.find('.button').click(function(e) {
					...
					if (val && val !== "http://") {
						...
						} else editor.execCommand("createlink", val);
					}
		...
		quote: {
			exec: function(caller, html, author) {
				...
				else if (this.getRangeHelper().selectedHtml() === "") end = '<br />' + end;
		...
		emoticon: {
			exec: function(caller) {
				var appendEmoticon, editor = this,
				...
				if (line.children().length > 0) content.append(line);

				if (editor.opts.emoticons.more) {
					var more = $(
					this._('<a class="sceditor-more">{0}</a>', this._("More"))).click(function() {
			...
			keyPress: function(e) {
				...
				if (this.opts.toolbar.indexOf('emoticon') === -1) return;
				...
				if (!editor.longestEmoticonCode) editor.longestEmoticonCode = editor.EmoticonsCache[editor.EmoticonsCache.length - 1][0].length;
				...
				if (editor.getRangeHelper().raplaceKeyword(editor.EmoticonsCache, true, true, editor.longestEmoticonCode, editor.opts.emoticonsCompat, curChar)) {
					if (/^\s$/.test(curChar) && editor.opts.emoticonsCompat) return true;
			...
			_dropDown: function(editor, caller, handleIdFunc) {
				...
				content.find('.button').click(function(e) {
					...
					if (val !== "") {
						...
						if (/^[a-zA-Z0-9_\-]{11}$/.test(val)) handleIdFunc(val);
						else alert('Invalid YouTube video');
					}
		...
		ltr: {
			exec: function() {
				...
				if (!elm || $elm.is('body')) {
					...
					if (!elm || $elm.is('body')) return;
				}

				if ($elm.css('direction') === 'ltr') $(elm).css('direction', '');
				else $(elm).attr('direction', 'ltr');
			},
		...
		rtl: {
			exec: function() {
				...
				if ($elm.css('direction') === 'rtl') $(elm).css('direction', '');
				else $(elm).css('direction', 'rtl');
			},
	...
	$.sceditor.rangeHelper = function(w, d) {
		var win, doc, init, _createMarker, _getOuterText, _selectOuterText, isW3C = true,
			startMarker = "sceditor-start-marker",
			endMarker = "sceditor-end-marker",
			characterStr = 'character',
			// Used to improve minification
			base = this;
		...
		base.insertHTML = function(html, endHTML) {
			...
			if (endHTML) html += base.selectedHtml() + endHTML;
		...
		base.insertNode = function(node, endNode) {
			if (isW3C) {
				var selection, selectAfter, toInsert = doc.createDocumentFragment(),
			...
			} else base.insertHTML(node.outerHTML, endNode ? endNode.outerHTML : null);
		};
		...
		base.cloneSelected = function() {
			...
			if (range) return isW3C ? range.cloneRange() : range.duplicate();
		};
		...
		base.selectedRange = function() {
			var r, sel = isW3C ? win.getSelection() : doc.selection;
		...
		base.selectedHtml = function() {
			...
			// IE < 9
			if (!isW3C && range.text !== '' && range.htmlText) return range.htmlText;
		...
		base.parentNode = function() {
			...
			if (range) return isW3C ? range.commonAncestorContainer : range.parentElement();
		};
		...
		base.getFirstBlockParent = function() {
			var func = function(node) {
					if (!$.sceditor.dom.isInline(node)) return node;
		...
		base.restoreRange = function() {
			var marker, range = base.selectedRange(),
		...
		_getOuterText = function(before, length) {
			...
			if (before) {
				...
				if (!isW3C) {
					...
					ret = range.text;
				} else ret = range.startContainer.textContent.substr(range.startOffset, length);
			}
		...
		base.raplaceKeyword = function(rep, includeAfter, repSorted, longestKey, requireWhiteSpace, curChar) {
			if (!repSorted) rep.sort(function(a, b) {
				return a.length - b.length;
			});

			var before, after, str, i, start, left, pat, lookStart, maxKeyLen = longestKey || rep[rep.length - 1][0].length;
			...
			if (includeAfter) after = _getOuterText(false, maxKeyLen);
			...
			while (i--) {
				...
				if (requireWhiteSpace)--lookStart;

				lookStart = Math.max(0, lookStart);

				if ((!requireWhiteSpace && (start = str.indexOf(rep[i][0], lookStart)) > -1) || (requireWhiteSpace && (start = str.substr(lookStart).search(pat)) > -1)) {
					if (requireWhiteSpace) start += lookStart + 1;

					// make sure the substr is between before and after not entierly in one
					// or the other
					if (start > before.length || start + rep[i][0].length + (requireWhiteSpace ? 1 : 0) < before.length) continue;
	...
	/** @lends jQuery.sceditor.dom */ {
		...
		traverse: function(node, func, innermostFirst, siblingsOnly, reverse) {
			if (node) {
				...
				while (node != null) {
					...
					if (!innermostFirst && func(node) === false) return false;

					// traverse all children
					if (!siblingsOnly && this.traverse(node, func, innermostFirst, siblingsOnly, reverse) === false) return false;
		...
		isInline: function(elm, includeCodeAsBlock) {
			if (!elm || elm.nodeType !== 1) return true;

			elm = elm.tagName.toLowerCase();

			if (elm === 'code') return !includeCodeAsBlock;
		...
		removeWhiteSpace: function(root) {
			...
			this.traverse(root, function(node) {
				...
				if (node.nodeType === 3 && $(node).parents('code, pre').length === 0 && nodeValue) {
					...
					if (!/\S|\u00A0/.test(nodeValue)) node.nodeValue = " ";
					// replace multiple spaces inbetween non-white space with a single space
					else if (regex.test(nodeValue)) node.nodeValue = nodeValue.replace(regex, " ");
				}
			});
		},
		...
		extractContents: function(startNode, endNode) {
			...
			return (function extract(root) {
				...
				base.traverse(root, function(node) {
					...
					if (node === startNode) startReached = true;
	...
	/** @lends jQuery.sceditor.command */ {
		...
		remove: function(name) {
			if ($.sceditor.commands[name]) delete $.sceditor.commands[name];
	...
	$.sceditor.defaultOptions = {
		 ...
		 * @type {String}
		 */
		toolbar: "bold,italic,underline,strike,subscript,superscript|left,center,right,justify|" + "font,size,color,removeformat|cut,copy,paste,pastetext|bulletlist,orderedlist|" + "table|code,quote|horizontalrule,image,email,link,unlink|emoticon,youtube,date,time|" + "ltr,rtl|print,source",
	...
	$.fn.sceditor = function(options) {
		...
		if (!options.runWithoutWysiwygSupport && !$.sceditor.isWysiwygSupported) return;

		this.each(function() {

			$this = this.jquery ? this : $(this);
			// Don't allow the editor to be initilised on it's own source editor
			if ($this.parents('.sceditor-container').length > 0) return;

			// Add state of instance to ret if that is what options is set to
			if (options === "state") ret.push( !! $this.data('sceditor'));
			else if (options === "instance") ret.push($this.data('sceditor'));
				...
				else(new $.sceditor(this, options));
			}
