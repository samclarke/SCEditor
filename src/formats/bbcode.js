...
function bbcodeFormat() {
		...
		each(bbcodeHandlers, function (bbcode, handler) {
			...
			if (styles) {
				each(styles, function (style, values) {
					stylesToBBCodes.push([style, values, handler.format]);
				});
			}
		});
	};
	 ...
	 * @private
	 */
	function handleStyles(element, content) {
		for (const [property, values, format] of stylesToBBCodes) {
			const styleValue = dom.getStyle(element, property);
	 ...
	 * @private
	 */
	function handleTags(element, content) {
		const
			tag        = element.nodeName.toLowerCase(),
			thisTag    = tagsToBBCodes[tag],
			matchAttrs = ([attrib, values]) => {
				var a = attr(element, attrib);
				return a && (!values || values.includes(a));
			};

		if (thisTag) {
			// loop all bbcodes for this tag
			for (const [bbcodeAttribs, attrMatch, format] of thisTag) {
				// Skip if the element doesn't have the attribute or the
				// attribute doesn't match one of the required values
				let fn = Array.prototype[attrMatch === 1
					? 'every' : 'some'];
	...
	base.init = function () {
		...
		this.commands = extend(
			true, {}, bbcodeCommands, this.commands
		);
