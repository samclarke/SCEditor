(function(d){d.sceditor=function(c,b){var a=this,e=d(c),f=null,h=null,g=null,i=null,j=null,l=null,m=!1,q=[],n=null,r=null,I=[],o,p,w,x,y,v,z,A,B,C,D,E,F,k,G,t,u,H,s;a.commands=d.sceditor.commands;A=function(){j=d("<textarea></textarea>").hide();g=d('<iframe frameborder="0"></iframe>');"https:"===window.location.protocol&&g.attr("src","javascript:false");f.append(g).append(j);i=g[0];u(e.width());t(e.height());k().designMode="On";k().open();k().write('<html><head><link rel="stylesheet" type="text/css" href="'+
a.options.style+'" /></head><body></body></html>');k().close();k().designMode="On";d(k()).find("body").keypress(v);d(k()).keypress(v);d(k()).mousedown(z);d(k()).bind("beforedeactivate keypress",y)};B=function(){var b=function(b){x(d(this),a.commands[d(this).data("sceditor-command")]);b.preventDefault()};h=d('<div class="sceditor-toolbar" />');for(var c=a.options.toolbar.split("|"),e=0;e<c.length;e++){for(var l=d('<div class="sceditor-group" />'),m=c[e].split(","),g=0;g<m.length;g++)if(a.commands.hasOwnProperty(m[g])){var i=
d('<a class="sceditor-button sceditor-button-'+m[g]+' " unselectable="on"><div unselectable="on" /></a>');a.commands[m[g]].hasOwnProperty("tooltip")&&i.attr("title",a._(a.commands[m[g]].tooltip));a.commands[m[g]].exec?i.data("sceditor-wysiwygmode",!0):i.addClass("disabled");a.commands[m[g]].txtExec&&i.data("sceditor-txtmode",!0);i.data("sceditor-command",m[g]);i.click(b);l.append(i)}h.append(l)}null===a.options.toolbarContainer?f.append(h):d(a.options.toolbarContainer).append(h)};C=function(){d.each(a.commands,
function(a,b){"undefined"!==typeof b.keyPress&&q.push(b.keyPress)})};u=function(a){f.width(a);g.width(a);g.width(a+(a-g.outerWidth(!0)));j.width(a);j.width(a+(a-j.outerWidth(!0)))};t=function(b){f.height(b);b-=null===a.options.toolbarContainer?h.outerHeight():0;g.height(b);g.height(b+(b-g.outerHeight(!0)));j.height(b);j.height(b+(b-j.outerHeight(!0)))};D=function(){var b=d('<div class="sceditor-grip" />'),c=d('<div class="sceditor-resize-cover" />'),e=0,m=0,l=0,g=0,h=f.width(),i=f.height(),j=!1,q,
n,o,p,k;q=null==a.options.resizeMinHeight?i/2:a.options.resizeMinHeight;n=null==a.options.resizeMaxHeight?2*i:a.options.resizeMaxHeight;o=null==a.options.resizeMinWidth?h/2:a.options.resizeMinWidth;p=null==a.options.resizeMaxWidth?2*h:a.options.resizeMaxWidth;k=function(a){var b=g+(a.pageY-m),d=l+(a.pageX-e);d>=o&&(0>p||d<=p)&&u(d);b>=q&&(0>n||b<=n)&&t(b);a.preventDefault()};f.append(b);f.append(c.hide());b.mousedown(function(a){e=a.pageX;m=a.pageY;l=f.width();g=f.height();j=!0;c.show();d(document).bind("mousemove",
k);a.preventDefault()});d(document).mouseup(function(a){j&&(j=!1,c.hide(),d(document).unbind("mousemove",k),a.preventDefault())})};F=function(){var b=d.extend({},a.options.emoticons.more,a.options.emoticons.dropdown,a.options.emoticons.hidden);d.each(b,function(a,b){var d=document.createElement("img");d.src=b;I.push(d)})};a.createDropDown=function(b,c,e,f){a.closeDropDown();!1!==f&&(e=d(e),e.find(":not(input,textarea)").filter(function(){return 1===this.nodeType}).attr("unselectable","on"));l=d('<div class="sceditor-dropdown sceditor-'+
c+'" />').css({top:b.offset().top,left:b.offset().left}).append(e);l.appendTo(d("body"));m=!0;l.click(function(a){a.stopPropagation()})};E=function(b){!m&&3!==b.which&&a.closeDropDown();m=!1};a.closeDropDown=function(b){null!==l&&(l.remove(),l=null);!0===b&&a.focus()};k=function(){return i.contentDocument?i.contentDocument:i.contentWindow&&i.contentWindow.document?i.contentWindow.document:i.document?i.document:null};a.wysiwygEditorInsertHtml=function(b,c,e){a.focus();if(e||!(d(o()).is("code")||0!==
...
b);n=null}};a.wysiwygEditorInsertText=function(b){b=b.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/ /g,"&nbsp;").replace(/\r\n|\r/g,"\n").replace(/\n/g,"<br />");a.wysiwygEditorInsertHtml("<span>"+b+"</span>")};p=function(){var a;i.contentWindow&&i.contentWindow.getSelection?a=i.contentWindow.getSelection():k().selection&&(a=k().selection);a.getRangeAt&&0>=a.rangeCount&&a.addRange(k().createRange());return a.getRangeAt?a.getRangeAt(0):a.createRange?a.createRange():null};
...
"\\$&").replace("<","&lt;").replace(">","&gt;")};s=function(){return j.is(":visible")};a.toggleTextMode=function(){s()?a.setWysiwygEditorValue(a.getTextareaValue()):a.setTextareaValue(a.getWysiwygEditorValue());h.find(".sceditor-button").removeClass("disabled");n=null;j.toggle();g.toggle();h.find(".sceditor-button").each(function(){var a=d(this);s()&&!a.data("sceditor-txtmode")?a.addClass("disabled"):!s()&&!a.data("sceditor-wysiwygmode")&&a.addClass("disabled")})};x=function(b,c){s()?c.hasOwnProperty("txtExec")&&
...
"undefined"!==typeof a.commands[b]&&"undefined"!==typeof a.commands[b].errorMessage&&alert(a._(a.commands[b].errorMessage))}};v=function(b){a.closeDropDown();var c=o(),c=d(c);if(13===b.which&&(c.is("code, blockquote")||0!==c.parents("code, blockquote").length))return a.wysiwygEditorInsertHtml("<br />",null,!0),!1;var e=k();d.sceditor.dom.rTraverse(e.body,function(a){if(3===a.nodeType&&""!==a.nodeValue||"br"===a.nodeName.toLowerCase())return 0<d(a).parents("code, blockquote").length&&d(e.body).append(e.createElement("br")),
!1},!0);if(!(c.is("code")||0!==c.parents("code").length))for(c=q.length;c--;)q[c].call(a,b,i,j)};z=function(){a.closeDropDown()};G=function(){null!==a.options.height&&-1<a.options.height.toString().indexOf("%")&&t(f.parent().height()*(parseFloat(a.options.height)/100));null!==a.options.width&&-1<a.options.width.toString().indexOf("%")&&u(f.parent().width()*(parseFloat(a.options.width)/100))};a._=function(){var a=arguments;null!==r&&r[a[0]]&&(a[0]=r[a[0]]);return a[0].replace(/\{(\d+)\}/g,function(b,
c){return"undefined"!==typeof a[c-0+1]?a[c-0+1]:"{"+c+"}"})};H=function(){if(d.sceditor.locale[a.options.locale])r=d.sceditor.locale[a.options.locale];else{var b=a.options.locale.split("-");d.sceditor.locale[b[0]]&&(r=d.sceditor.locale[b[0]])}null!==r&&r.dateFormat&&(a.options.dateFormat=r.dateFormat)};(function(){e.data("sceditor",a);a.options=d.extend({},d.sceditor.defaultOptions,b);null!==a.options.locale&&"en"!==a.options.locale&&H();null!==a.options.height&&e.height(a.options.height);null!==
...
d(this).data("sceditor-fontsize"));b.closeDropDown(!0);a.preventDefault()},f=1;7>=f;f++)a.append(d('<a class="sceditor-fontsize-option" href="#"><font size="'+f+'">'+f+"</font></a>").data("sceditor-fontsize",f).click(e));b.createDropDown(c,"fontsize-picker",a)},tooltip:"Font Size"},color:{exec:function(c){for(var b=this,a={r:255,g:255,b:255},e=d("<div />"),f=this.options.colors?this.options.colors.split("|"):Array(21),h=[],g=0,i=0;i<f.length;++i){var j="undefined"!==typeof f[i]?f[i].split(","):Array(21);
h[g++]='<div class="sceditor-color-column">';for(var l=0;l<j.length;++l){var m="undefined"!==typeof j[l]?j[l]:"#"+a.r.toString(16)+a.g.toString(16)+a.b.toString(16);h[g++]='<a href="#" class="sceditor-color-option" style="background-color: '+m+'" data-color="'+m+'"></a>';a=0===l%5?{r:a.r,g:a.g-51,b:255}:{r:a.r,g:a.g,b:a.b-51}}h[g++]="</div>";a=0===i%5?{r:a.r-51,g:255,b:255}:{r:a.r,g:255,b:255}}e.append(h.join("")).find("a").click(function(a){b.execCommand("forecolor",d(this).attr("data-color"));b.closeDropDown(!0);
...
a.append(d(this._('<div><input type="button" class="button" value="{0}" /></div>',this._("Insert"))).click(function(a){var c=d(this).parent("form").find("#rows").val()-0,h=d(this).parent("form").find("#cols").val()-0,g="<table>";if(!(1>c||1>h)){for(var i=0;i<c;i++){for(var g=g+"<tr>",j=0;j<h;j++)g=d.browser.msie?g+"<td></td>":g+'<td><br class="sceditor-ignore" /></td>';g+="</tr>"}b.wysiwygEditorInsertHtml(g+"</table>");b.closeDropDown(!0);a.preventDefault()}}));b.createDropDown(c,"inserttable",a)},
...
/\s/.test(h):!1,i=a.startContainer.textContent.substr(0,a.startOffset),j=a.startContainer.textContent.substr(a.startOffset,f.longestEmoticonCode),l=i+h+j;d.each(f.allEmoticonCache,function(d,i){if(-1<(e=l.indexOf(d,a.startOffset-d.length))){if(f.options.emoticonsCompat){var n=RegExp("(\\s{1})"+f.regexEscape(d)+"(?=\\s{1})","gm");if(g&&-1<(h+j).indexOf(d)||!n.test(l))return}a=a.cloneRange();a.setStart(a.startContainer,e);a.setEnd(a.startContainer,e+d.length-(g?0:1));a.deleteContents();n=b.contentDocument.createElement("div");
n.innerHTML='<img src="'+i+'" data-sceditor-emoticon="'+d+'" />';n=n.children[0];a.insertNode(n);a=a.cloneRange();a.setStartAfter(n);b.contentWindow.getSelection().removeAllRanges();b.contentWindow.getSelection().addRange(a);g||(c.preventDefault(),c.stopPropagation());return!1}})}},tooltip:"Insert an emoticon"},youtube:{exec:function(c){var b=this,a=d(this._('<form><div><label for="link">{0}</label> <input type="text" id="link" value="http://" /></div></form>',this._("Video URL:"))).submit(function(){return!1});
...
c)})}})(jQuery);(function(d){d.sceditorBBCodePlugin=function(c,b){var a=this,e,f,h,g;a.bbcodes=d.sceditorBBCodePlugin.bbcodes;var i={},j={};e=function(c,b){var e;d.each(j,function(f,g){if(!("a"===c.get(0).nodeName.toLowerCase()&&("color"===f||"text-decoration"===f))&&!("code"===c.get(0).nodeName.toLowerCase()&&"font-family"===f))e=c.css(f),c.parent().css(f)!==e&&d.each(g,function(f,g){if(null===g||-1<d.inArray(e.toString(),g))b=d.isFunction(a.bbcodes[f].format)?a.bbcodes[f].format.call(a,c,b):h(a.bbcodes[f].format,
b)})});return b};f=function(c,b){var e=c.get(0).nodeName.toLowerCase();if(c.hasClass("sceditor-ignore"))return b;i[e]&&d.each(i[e],function(e,f){if(null!==f){var g=!1;d.each(f,function(a,b){if(!("undefined"===typeof c.attr(a)||null!==b&&0>d.inArray(c.attr(a),b)))return g=!0,!1});if(!g)return}b=d.isFunction(a.bbcodes[e].format)?a.bbcodes[e].format.call(a,c,b):h(a.bbcodes[e].format,b)});/^(br|div|p)$/.test(e)&&(b+="\n");return b};h=function(){var a=arguments;return a[0].replace(/\{(\d+)\}/g,function(c,
b){return"undefined"!==typeof a[b-0+1]?a[b-0+1]:"{"+b+"}"})};a.stripQuotes=function(a){return a.replace(/^["']+/,"").replace(/["']+$/,"")};a.getHtmlHandler=function(c,b){return a.elementToBbcode(b)};a.elementToBbcode=function(a){return function q(a){var b="";d.sceditor.dom.traverse(a,function(a){var c=d(a),h="";3!==a.nodeType?("iframe"!==a.nodeName.toLowerCase()&&(h=c.is("code")?g(c):q(a)),c.is("code")||(h=e(c,h)),b+=f(c,h)):b+=a.nodeValue},!1,!0);return b}(a.get(0))};g=function(a){var b="";if("undefined"===
typeof a.get(0))return"";if(3===a.get(0).nodeType)return a.get(0).nodeValue;a.is("br")?b="\n":0<a.contents().length&&d.each(a.contents(),function(){b+=g(d(this))});return b};a.getTextHandler=function(b){for(var c=/\[([^\[\s=]*?)(?:([^\[]*?))?\]((?:[\s\S(?!=\[\\\1)](?!\[\1))*?)\[\/(\1)\]/g,e=/(\S+)=((?:(?:(["'])(?:\\\3|[^\3])*?\3))|(?:[^'"\s]+))/g,f,g=function(b,c,f,g){var i={},j;if(f)if(f=d.trim(f),"="===f.charAt(0)&&1>=f.split("=").length-1||"url"===c)i.defaultAttr=a.stripQuotes(f.substr(1));else for("="===
f.charAt(0)&&(f="defaultAttr"+f);j=e.exec(f);)i[j[1].toLowerCase()]=a.stripQuotes(j[2]);return!a.bbcodes[c]?b:d.isFunction(a.bbcodes[c].html)?a.bbcodes[c].html.call(a,c,i,g):h(a.bbcodes[c].html,g)},b=b.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\r/g,"").replace(/(\[\/?(?:left|center|right|justify)\])\n/g,"$1").replace(/\n/g,"<span><br /></span>");b!==f;)f=b,b=b.replace(c,g);b=b.replace(/\[hr\]/gi,"<hr>");return b.replace(/ {2}(?=([^<\>]*?<|[^<\>]*?$))/g," &nbsp;")};a.options=
d.extend({},d.sceditor.defaultOptions,d.sceditorBBCodePlugin.defaultOptions,b);(function(){d.each(a.bbcodes,function(b){"undefined"!==typeof a.bbcodes[b].tags&&d.each(a.bbcodes[b].tags,function(a,c){i[a]=i[a]||{};i[a][b]=c});"undefined"!==typeof a.bbcodes[b].styles&&d.each(a.bbcodes[b].styles,function(a,c){j[a]=j[a]||{};j[a][b]=c})})})();new d.sceditor(c,d.extend({},a.options,{getHtmlHandler:a.getHtmlHandler,getTextHandler:a.getTextHandler}))};d.sceditorBBCodePlugin.bbcodes={b:{styles:{"font-weight":"bold,bolder,401,700,800,900".split(",")},
format:"[b]{0}[/b]",html:"<strong>{0}</strong>"},i:{styles:{"font-style":["italic","oblique"]},format:"[i]{0}[/i]",html:"<em>{0}</em>"},u:{styles:{"text-decoration":["underline"]},format:"[u]{0}[/u]",html:"<u>{0}</u>"},s:{styles:{"text-decoration":["line-through"]},format:"[s]{0}[/s]",html:"<s>{0}</s>"},sub:{tags:{sub:null},format:"[sub]{0}[/sub]",html:"<sub>{0}</sub>"},sup:{tags:{sup:null},format:"[sup]{0}[/sup]",html:"<sup>{0}</sup>"},font:{styles:{"font-family":null},format:function(c,b){return c.css("font-family")===
this.options.defaultFont?b:"[font="+this.stripQuotes(c.css("font-family"))+"]"+b+"[/font]"},html:function(c,b,a){return'<font face="'+b.defaultAttr+'">'+a+"</font>"}},size:{styles:{"font-size":null},format:function(c,b){var a=c.css("fontSize"),d=1;if(a===this.options.defaultSize)return b;-1<a.indexOf("px")?(a=a.replace("px","")-0,12<a&&(d=2),15<a&&(d=3),17<a&&(d=4),23<a&&(d=5),31<a&&(d=6),47<a&&(d=7)):d=a;return"[size="+d+"]"+b+"[/size]"},html:function(c,b,a){return'<font size="'+b.defaultAttr+'">'+
a+"</font>"}},color:{styles:{color:null},format:function(c,b){var a=function(a){function b(a){a=parseInt(a,10);if(isNaN(a))return"00";a=Math.max(0,Math.min(a,255)).toString(16);return 2>a.length?"0"+a:a}var c;return(c=a.match(/rgb\((\d+),\s*?(\d+),\s*?(\d+)\)/i))?"#"+b(c[1])+b(c[2]-0)+b(c[3]-0):a}(c.css("color"));return a===this.options.defaultColor?b:"[color="+a+"]"+b+"[/color]"},html:function(c,b,a){return'<font color="'+b.defaultAttr+'">'+a+"</font>"}},ul:{tags:{ul:null},validChildren:["li"],isBlock:!0,
format:"[ul]{0}[/ul]",html:"<ul>{0}</ul>"},list:{html:"<ul>{0}</ul>"},ol:{tags:{ol:null},validChildren:["li"],isBlock:!0,format:"[ol]{0}[/ol]",html:"<ol>{0}</ol>"},li:{tags:{li:null},format:"[li]{0}[/li]",html:"<li>{0}</li>"},"*":{html:"<li>{0}</li>"},table:{tags:{table:null},validChildren:["tr","th"],format:"[table]{0}[/table]",html:"<table>{0}</table>"},tr:{tags:{tr:null},validChildren:["td"],format:"[tr]{0}[/tr]",html:"<tr>{0}</tr>"},th:{tags:{th:null},validChildren:["td"],format:"[th]{0}[/th]",
...
e.elementToBbcode(d(this))}));return"[quote"+a+"]"+b+"[/quote]"},html:function(c,b,a){"undefined"!==typeof b.defaultAttr&&(a="<cite>"+b.defaultAttr+"</cite>"+a);return"<blockquote>"+a+"</blockquote>"}},code:{tags:{code:null},validChildren:[],isBlock:!0,format:"[code]{0}[/code]",html:"<code>{0}</code>"},left:{styles:{"text-align":["left"]},format:"[left]{0}[/left]",html:'<div align="left">{0}</div>'},center:{styles:{"text-align":["center"]},format:"[center]{0}[/center]",html:'<div align="center">{0}</div>'},
right:{styles:{"text-align":["right"]},format:"[right]{0}[/right]",html:'<div align="right">{0}</div>'},justify:{styles:{"text-align":["justify"]},format:"[justify]{0}[/justify]",html:'<div align="justify">{0}</div>'},youtube:{tags:{iframe:{"data-youtube-id":null},validChildren:[]},format:function(c,b){return!c.attr("data-youtube-id")?b:"[youtube]"+c.attr("data-youtube-id")+"[/youtube]"},html:'<iframe width="560" height="315" src="http://www.youtube.com/embed/{0}" data-youtube-id="{0}" frameborder="0" allowfullscreen></iframe>'}};
