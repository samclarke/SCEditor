(function(d){d.sceditor=function(c,b){var a=this,e=d(c),f=null,h=null,g=null,i=null,k=null,j=null,n=!1,q=[],m=null,o=null,I=[],p,r,w,x,y,v,z,A,B,C,D,E,F,l,G,t,u,H,s;a.commands=d.sceditor.commands;A=function(){k=d("<textarea></textarea>").hide();g=d('<iframe frameborder="0"></iframe>');"https:"===window.location.protocol&&g.attr("src","javascript:false");f.append(g).append(k);i=g[0];u(e.width());t(e.height());l().designMode="On";l().open();l().write('<html><head><link rel="stylesheet" type="text/css" href="'+
a.options.style+'" /></head><body></body></html>');l().close();l().designMode="On";d(l()).find("body").keypress(v);d(l()).keypress(v);d(l()).mousedown(z);d(l()).bind("beforedeactivate keypress",y)};B=function(){var b=function(b){x(d(this),a.commands[d(this).data("sceditor-command")]);b.preventDefault()};h=d('<div class="sceditor-toolbar" />');for(var c=a.options.toolbar.split("|"),e=0;e<c.length;e++){for(var n=d('<div class="sceditor-group" />'),j=c[e].split(","),g=0;g<j.length;g++)if(a.commands.hasOwnProperty(j[g])){var i=
...
/\s/.test(h):!1,i=a.startContainer.textContent.substr(0,a.startOffset),k=a.startContainer.textContent.substr(a.startOffset,f.longestEmoticonCode),j=i+h+k;d.each(f.allEmoticonCache,function(d,i){if(-1<(e=j.indexOf(d,a.startOffset-d.length))){if(f.options.emoticonsCompat){var m=RegExp("(\\s{1})"+f.regexEscape(d)+"(?=\\s{1})","gm");if(g&&-1<(h+k).indexOf(d)||!m.test(j))return}a=a.cloneRange();a.setStart(a.startContainer,e);a.setEnd(a.startContainer,e+d.length-(g?0:1));a.deleteContents();m=b.contentDocument.createElement("div");
m.innerHTML='<img src="'+i+'" data-sceditor-emoticon="'+d+'" />';m=m.children[0];a.insertNode(m);a=a.cloneRange();a.setStartAfter(m);b.contentWindow.getSelection().removeAllRanges();b.contentWindow.getSelection().addRange(a);g||(c.preventDefault(),c.stopPropagation());return!1}})}},tooltip:"Insert an emoticon"},youtube:{exec:function(c){var b=this,a=d(this._('<form><div><label for="link">{0}</label> <input type="text" id="link" value="http://" /></div></form>',this._("Video URL:"))).submit(function(){return!1});
...
h=function(){var a=arguments;return a[0].replace(/\{(\d+)\}/g,function(c,b){return"undefined"!==typeof a[b-0+1]?a[b-0+1]:"{"+b+"}"})};a.stripQuotes=function(a){return a.replace(/^["']+/,"").replace(/["']+$/,"")};a.getHtmlHandler=function(b,c){return a.elementToBbcode(c)};a.elementToBbcode=function(a){return function q(a,c){var b="";d.sceditor.dom.traverse(a,function(a){var g=d(a),h="",i=a.nodeName.toLowerCase(),j=k[i],m=!0;"object"===typeof c&&((m=-1<d.inArray(i,c))||(j=c));3!==a.nodeType?g.hasClass("sceditor-ignore")||
("iframe"!==i&&(h=q(a,j)),m?(g.is("code")||(h=e(g,h),h=f(g,h),h=e(g,h,!0)),b+=f(g,h,!0)):b+=h):b+=a.nodeValue},!1,!0);return b}(a.get(0))};a.getTextHandler=function(b){for(var c=/\[([^\[\s=]*?)(?:([^\[]*?))?\]((?:[\s\S(?!=\[\\\1)](?!\[\1))*?)\[\/(\1)\]/g,e=/(\S+)=((?:(?:(["'])(?:\\\3|[^\3])*?\3))|(?:[^'"\s]+))/g,f,g=function(b,c,f,g){var i={},j;if(f)if(f=d.trim(f),"="===f.charAt(0)&&1>=f.split("=").length-1||"url"===c)i.defaultAttr=a.stripQuotes(f.substr(1));else for("="===f.charAt(0)&&(f="defaultAttr"+
